...
import src.model_training.core.config as config

from src.model_training.models.ResNet import ResNet
from src.model_training.utils.save_model import save_model
from src.model_training.data_loader import ImageDataLoader
...
def train(model: nn.Module, save_name: str):
    ...
    for epoch in range(config.EPOCHS):
        ...
        print(f"Train loss: {epoch_train_loss:.5f} | Train acc: {epoch_train_acc*100:.2f}%")
        end_time = time.time()
        ...
        print(f"Test loss: {epoch_test_loss:.5f} | Test acc: {epoch_test_acc*100:.2f}%\n")
